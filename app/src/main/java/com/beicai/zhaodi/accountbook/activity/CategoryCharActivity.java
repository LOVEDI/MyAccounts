...
public class CategoryCharActivity extends FrameActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initVariable();
        View pie_view = categroryStatistics();
        appendMainBody(pie_view);
        removeBottomBox();
        setTitle();
    }
    //类别统计
    private View categroryStatistics() {
        ...
        //调用自动以构造渲染器
        DefaultRenderer defaultRender = bindCategoryRenderer(color);
        //获取数据源
        CategorySeries categorySeries = buildCategoryDataset("测试饼图", categoryTotalList);
        //获取一个饼图的View
        View pie_view = ChartFactory.getPieChartView(this, categorySeries, defaultRender);
        return  pie_view;
    }
    private DefaultRenderer bindCategoryRenderer(int colors[]){
    ...
    private void setTitle(){
        String title = getString(R.string.category_total);
